...
define( 'TRACKFREE_VERSION', '3.1.2' );
    ...
    function trackfree_enqueue($hook)
    {
        ...
        if ($hook === 'toplevel_page_trackfree') {
            ...
            wp_enqueue_script(
                ...
                plugin_dir_url(__FILE__) . 'dist/static/js/main.3eb6a52c.js',
                array(),
                TRACKFREE_VERSION,
                true
            );
            ...
            if ($shipment_details) {
                ...
                foreach ($shipment_details as $key => $shipment_values) {
                    if ($shipment_values['tracking_num'] == $tracking_number) {
                        $statuses[] = $tracking_status ? $tracking_status : $shipment_values['status'];
                        $tack_details[$key] = array(
                ...
                update_post_meta($order_id, '_trackfree_shipment_details', wc_clean($tack_details));
